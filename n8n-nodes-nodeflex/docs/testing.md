...
| `nodes/ExecutePrivateWorkflow/ExecutePrivateWorkflow.node.test.ts` | EW-04, EW-05, EW-06, EW-08, EW-10, EW-16, EW-19 |
...
- `tests/integration/executeTriggerRespondGetResult.test.ts` — a full round trip for a JSON payload through all four nodes' real production code: Execute sends it, a really-connected Trigger (real SignalR client, real hub registration) receives and emits it, Respond posts a response, GetResult reads it back. This is the template the full per-data-type matrix (sections 1–4, once scoped) will replicate.
...
| PWT-03 | Integration | Activation fails — hub unreachable | Credential with a syntactically valid but non-existent API key | — | Attempt activation | Throws `NodeOperationError`: "Hub URL is unavailable. Hub service is down." (or equivalent hub-rejection error) | Activation error message matches |
...
| EW-17 | Integration | Hub service unavailable | Credential with invalid API key | any | Throws `NodeOperationError`: "Hub service is unavailable." | Execution fails with that message |
| EW-18 | Integration | Hub 5xx response | Simulate/force a hub 500 (requires hub-side fault injection, or test against a known-bad path) | any | Throws `NodeOperationError`: `Hub error (500)` | Execution fails, message matches pattern `Hub error (\d{3})` |
...
| RW-16 | Unit | Binary — no binary data found (graceful, non-throwing) | Respond With = binary | 1 item with no `binary` key | Logs a warning; still sends/outputs `{ correlationId, status: 'Success' }` with `payload = null`; **does not throw** | Execution succeeds; output `json.status === 'Success'`, no `binary` key |
...
| RW-20 | Integration | Node failure reports "Failed" status to hub without masking the real error | Force a downstream error (e.g. Response Body = `FIX-JSON-INVALID`, respondWith = json) | any | Best-effort POST to hub with `status: 'Failed'` and the error message is attempted; node then still throws the original `NodeOperationError` ("Response Body must contain valid JSON") | Execution fails with the original message (not a hub-POST error); on the caller side, `GET Private Workflow Result` (or Execute's Completed output) reflects a non-`Completed` state |
...
| GR-05 | Unit | Correlation ID blank or unknown | Correlation ID = `` or a random UUID never issued by the hub | — | Hub responds without a recognizable `status`; throws `NodeOperationError`: "Hub response missing status field" | Execution fails with that message |
...
| GR-07 | Integration | Hub service unavailable | Credential with invalid API key | — | Throws `NodeOperationError`: "Hub service is unavailable." | Execution fails with that message |
| GR-08 | Integration | Hub info incomplete | (Requires a hub account misconfigured to omit `apiUrl`/`hubUrl`/`blobStorageUrl` — hub-side setup) | — | Throws `NodeOperationError`: "Hub service information is incomplete or unavailable." | Execution fails with that message |
...
| `text` | Respond (text) | Execute (Completed), Get Result | RW-13, EW-22, GR-10 |
...
- `ExecutePrivateWorkflow.node.ts` declares a `Text` property (`textValue`, shown when `payloadType === 'text'`) but the `Payload` (`payloadType`) parameter only offers `json` and `binary` options — `text` is unreachable from the UI. This property is currently dead code. Not covered by a test case above since it cannot be exercised; flagging here rather than silently omitting it.
