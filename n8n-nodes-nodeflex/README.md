...
4. Enter `n8n-nodes-nodeflex` in the package name field
...
```bash
cd ~/.n8n/custom
npm install n8n-nodes-nodeflex
```
...
```bash
docker run -it --rm \
  -e N8N_COMMUNITY_PACKAGES="n8n-nodes-nodeflex" \
  -p 5678:5678 \
  n8nio/n8n
```
...
```yaml
environment:
  - N8N_COMMUNITY_PACKAGES=n8n-nodes-nodeflex
```
